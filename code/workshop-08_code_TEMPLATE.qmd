...
```{r data-cleaning}
# INSERT COMMENT HERE
drought_exp_clean <- drought_exp |> # making new object 
  # cleaning names 
  clean_names() |> 
  # making new columns  
  ...
  # making sure species is a factor, first level should be lotus scoparius 
  mutate(species_name = as_factor(species_name),
         species_name = fct_relevel(species_name,
                                    ...
                                    "Grindelia camporum")) |> 
  # ensure water treatment is a factor, order them 
...
slice_sample(
  drought_exp_clean, # specifying data frame to display
...
ggplot(data = drought_exp_clean,
  ...
  labs(x = expression(Specific~leaf~area~(cm^2/g)),
       y = "Total mass (g)",
       title = "As Leaf area increases, total mass increases")
```
...
# model 0: null model
model0 <- lm(
  total_g ~ 1, # formula, indicating that there are no predictors 
...
AICc(model1,
     ...
     model7,
     model0) |> 
  arrange(AICc) # arranges models from lowest to highest: lowest is the most accurate model 
```

Look at the model summary:

```{r model-summary}
summary(model4) # drought stress is reference level, and lotus is reference level 
...
On average, plant masses in drought stressed conditions were 0.12 g $\pm$ 0.02 g less than plant masses in well watered conditions across species 
Across water treatments, all species have different (and higher) masses than *lotus scoparius* except *Penstemon centranthifolious*. 

# 5. Model predictions

```{r model-predictions}
model_predictions <- ggpredict(model4, # model object 
...
ggplot(data = drought_exp_clean,
  ...
  # in the final model
  geom_pointrange(data = model_predictions,
                  aes(y = predicted,
                      ymin = conf.low,
                      ymax = conf.high)) +
  # manually setting colors
